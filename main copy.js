	...
	$.post("http://pvamu.hazar.us/php/getRole.php",{username: userName}, function(data) {
		if(data == 3) {
			...
			$.post("http://pvamu.hazar.us/php/getUserInfo.php", function(data) {
				alert("From getUserInfo.php: "+data);
			});
		}
...
function unVerified(u) {
	...
	$(document).ready(function(){
		...
		$.post("http://pvamu.hazar.us/php/isVerified.php", function(data) {
			if(data == 1) {
...
$(document).ready(function(){
		...
		$("#fpasssub").click(function() {
			$("#loadpic4").show();
			var e = $("#pvemail").val();
			var emailString = /@pvamu.edu/g;
			var emailValidate = emailString.test(e)			
			
			if(e == "") {
				$("#loadpic4").hide();
				navigator.notification.alert('Please enter your PV email address',null,'Password Reset','Ok');
				return false;
			}
				...
				$.post("http://pvamu.hazar.us/php/forgotPassword.php",{email:e},function(data) {
					if(data ==1) {
						$("#loadpic4").hide();
						navigator.notification.alert('A password reset link has been sent to your email address.',null,'Password Reset','Ok');
						$("#pvemail").val("PVEmail");
						window.location.href = "index.html#page1";
					}
					else {
						$("#loadpic4").hide();
						navigator.notification.alert('No user with that email address exists.',null,'Password Reset','Ok');
						return false;
					}
				});	
			}
...
function addStudent(button) {
	if(button == 1) {
		...
		$.post("http://pvamu.hazar.us/php/addAdvisee.php",{studentEmail: Email},function(data) {
